...
__elf_load__(void *exe_ptr, int pmap_id)
{
	...
	//debug prints
	KERN_DEBUG("Attempting to load ELF with pmap_id=%d at exe_ptr=%p\n", pmap_id, exe_ptr);

	eh = (elfhdr *) exe;

	KERN_DEBUG("Checking ELF magic: 0x%x (expected 0x%x)\n", eh->e_magic, ELF_MAGIC);
	KERN_ASSERT(eh->e_magic == ELF_MAGIC);
		
	KERN_DEBUG("e_shstrndx = %d\n", eh->e_shstrndx);
	...
	KERN_DEBUG("BSS section found: base=0x%08x, size=0x%08x\n", bss_base, bss_size);
	...
	for (; ph < eph; ph++) {
		...
		KERN_DEBUG("Program Header - Type: %u, Offset: 0x%08x, Virtual Address: 0x%08x, File Size: 0x%08x\n", ph->p_type, ph->p_offset, ph->p_va, ph->p_filesz);
		...
		for(; va < eva; va += PAGESIZE, fa += PAGESIZE) {
			...
			KERN_DEBUG("Reserving page at virtual address 0x%08x with permissions 0x%08x\n", va, perm);

			if (va < rounddown(zva, PAGESIZE)) {
				/* copy a complete page */
				pt_copyout((void *) fa, pmap_id, va, PAGESIZE);
				KERN_DEBUG("Copying out full page from 0x%08lx to 0x%08lx\n", fa, va);
			} else if (va < zva && ph->p_filesz) {
				/* copy a partial page */
				pt_memset(pmap_id, va, 0, PAGESIZE);
				pt_copyout((void *) fa, pmap_id, va, zva-va);
				KERN_DEBUG("Copying out partial page from 0x%08lx to 0x%08lx\n", fa, va);
			} else {
				/* zero a page */
				pt_memset(pmap_id, va, 0, PAGESIZE);
				KERN_DEBUG("Zeroing out page at 0x%08lx\n", va);
			}
		}
	}
	KERN_DEBUG("Finished loading ELF program.\n");
}
...
extern uint8_t _binary___obj_user_myapplication_p1_start[];
extern uint8_t _binary___obj_user_myapplication_p2_start[];
extern uint8_t _binary___obj_user_myapplication_p3_start[];
...
const uint8_t * binaries[] = {
	...
	_binary___obj_user_vmm_vmm_start,
	_binary___obj_user_myapplication_p1_start,
	_binary___obj_user_myapplication_p2_start,
	_binary___obj_user_myapplication_p3_start,
};
...
const char * elf_names[] ={
	...
	TO_STRING(_binary___obj_user_vmm_vmm_start),
	TO_STRING(_binary___obj_user_myapplication_p1_start),
	TO_STRING(_binary___obj_user_myapplication_p2_start),
	TO_STRING(_binary___obj_user_myapplication_p3_start),
...
{
  KERN_DEBUG ("pmapid = %d\n", pmap_id);
  KERN_DEBUG("Loading ELF file with elf_id=%u\n", pmap_id);
	...
	if (loaded[i] == 0 && pmap_id == i)
	{
		__elf_load__ (_binary___obj_user_myapplication_p1_start, 4);
		loaded[i] = 1;
	...
	if (loaded[i] == 0 && pmap_id == i)
	{
		__elf_load__ (_binary___obj_user_myapplication_p2_start, 5);
		loaded[i] = 1;
	...
	if (loaded[i] == 0 && pmap_id == i)
	{
		__elf_load__ (_binary___obj_user_myapplication_p3_start, 6);
		loaded[i] = 1;
...
elf_preload(void)
{
	...
	ELF_LOC[i] = _binary___obj_user_myapplication_p1_start;
	ELF_ENTRY_LOC[i] = elf_entry(_binary___obj_user_myapplication_p1_start);
	i++;

	ELF_LOC[i] = _binary___obj_user_myapplication_p2_start;
	ELF_ENTRY_LOC[i] = elf_entry(_binary___obj_user_myapplication_p2_start);
	i++;

	ELF_LOC[i] = _binary___obj_user_myapplication_p3_start;
	ELF_ENTRY_LOC[i] = elf_entry(_binary___obj_user_myapplication_p3_start);
	i++;
	...
	KERN_INFO("Finished loading ELF table.\n");
}
